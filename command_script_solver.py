...
for i in range(num_meshes): 
    ...
    solution_time, num_of_dofs = penalty_solver(R,E,nu,penetration,penalty, mesh, boundary_markers)
    ...
    solution_time, num_of_dofs = snes_solver(R,E,nu,penetration,penalty, mesh, boundary_markers)
    time_snes[i] = solution_time
    # with XDMFFile("u_SNES_" + str(i) + ".xdmf") as xdmf:
    #     xdmf.write_checkpoint(u, "u", 0.0, XDMFFile.Encoding.HDF5, append=False)

    solution_time, num_of_dofs = nitsche_solver(R,E,nu,penetration, penalty_nitsche, mesh, boundary_markers)
    time_nitsche[i] = solution_time
    # with XDMFFile("u_NITSCHE_" + str(i) + ".xdmf") as xdmf:
    #     xdmf.write_checkpoint(u, "u", 0.0, XDMFFile.Encoding.HDF5, append=False)

    solution_time, num_of_dofs = nitscheRFP_solver(R,E,nu,penetration, penalty_nitsche_RFP, theta, mesh, boundary_markers)
