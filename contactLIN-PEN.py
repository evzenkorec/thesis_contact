...
Pi = psi*dx - dot(B, u)*dx - dot(T0, u)*ds(0) - dot(T1, u)*ds(1) + (penalty/2.0)*dot(maculay(-gap()),maculay(-gap()))*ds(2)
...
solve(F == 0, u, bc, J = J, solver_parameters={"newton_solver":{"relative_tolerance":1e-10},"newton_solver":{"maximum_iterations":100}})

# Solver "NonlinearVariationalProblem" alternative to "solve" - commented 
...
plt.ylabel('$y$')
plt.colorbar(graph1)
plt.savefig("displacementL-PEN.pdf", format="pdf")
...
plt.xlabel('$x$')
plt.ylabel('$y$')
plt.savefig("meshL-PEN.pdf", format="pdf")
...
plt.colorbar(graph3)
plt.savefig("stressL-PEN.pdf", format="pdf")
...
plt.colorbar(graph4)
plt.savefig("MAGdisplacementL-PEN.pdf", format="pdf")
