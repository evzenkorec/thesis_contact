"""This program solves contact problem of linear elastic semi-circle
 which is in contact with rigid plane using the Nitsche's method. Semi-circle initially "lies" on
...
plt.ylabel('$y$')
plt.colorbar(graph1)
plt.savefig("displacementL-NITSCHE.pdf", format="pdf")
...
plt.xlabel('$x$')
plt.ylabel('$y$')
plt.savefig("meshL-NITSCHE.pdf", format="pdf")
...
plt.colorbar(graph3)
plt.savefig("stressL-NITSCHE.pdf", format="pdf")
