"""This program solves contact problem of linear elastic semi-circle
 which is in contact with rigid plane using the Lagrange multiplier method. 
...
Pi = psi*dx - dot(B, u)*dx - dot(T0, u)*ds(0) - dot(T1, u)*ds(1) + dot(-maculay(-Lm),(gap()))*ds(2)
...
plt.ylabel('$y$')
plt.colorbar(graph1)
plt.savefig("displacementL-LM.pdf", format="pdf")
...
plt.xlabel('$x$')
plt.ylabel('$y$')
plt.savefig("meshL-LM.pdf", format="pdf")
...
plt.colorbar(graph3)
plt.savefig("stressL-LM.pdf", format="pdf")
...
plt.colorbar(graph4)
plt.savefig("MAGdisplacementL-LM.pdf", format="pdf")
